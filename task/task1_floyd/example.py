...
W: Final[list[list[int]]] = [
    ...
    [INF, 0, 1, INF, 1, 5],                 # 1번 정점은 2번, 4번, 5번 정점과 연결되어 있다.
    [INF, 9, 0, 3, 2, INF],                 # 2번 정점은 1번, 3번, 4번 정점과 연결되어 있다.
    [INF, INF, INF, 0, 4, INF],             # 3번 정점은 4번 정점과 연결되어 있다.
    [INF, INF, INF, 2, 0, 3],               # 4번 정점은 3번, 5번 정점과 연결되어 있다.
    [INF, 3, INF, INF, INF, 0]              # 5번 정점은 1번 정점과 연결되어 있다.
]
# D와 P 행렬 생성
...
print("두 정점 (행, 열) 간의 경로의 가중치 합: ")         # 2. D, P 배열 출력
print_array(N, D)
print("경로 행렬: ")
print_array(N, P)
print()

print("v5와 v3 사이의 최단 경로: ", end='')
path(5 , 3)             # 3. 경로 출력
print()
